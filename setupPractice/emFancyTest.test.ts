...
let newInturns: Array<Inturns> = [
    ...
    new Inturns("Miss Sara Bellum", 1234567890, "Secretary")
]
...
let addEmp = async (newInturns) => {
    ...
    await employeePage.setInput(employeePage.nameField, Inturns.name)
    await employeePage.click(employeePage.phoneField)
    await employeePage.setInput(employeePage.phoneField, Inturns.phone)
...
describe("should add employees to employee manager", () => {
    test("can add emplyees using array", async () => {
